...
export default class GroupsStore extends BaseStore<Group> {
  ...
  @action
  fetchPage = async (params: PaginationParams | null | undefined): Promise<any> => {
  ...
  inCollection = (collectionId: string, query: string) => {
    const memberships = filter(this.rootStore.collectionGroupMemberships.orderedData, member => member.collectionId === collectionId);
    const groupIds = memberships.map(member => member.groupId);
    const groups = filter(this.orderedData, group => groupIds.includes(group.id));
...
function queriedGroups(groups, query) {
  return filter(groups, group => group.name.toLowerCase().match(query.toLowerCase()));
}