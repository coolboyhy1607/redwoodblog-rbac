...
export default class UsersStore extends BaseStore<User> {
  ...
  @computed
  get active(): User[] {
    return filter(this.orderedData, user => !user.isSuspended && user.lastActiveAt);
  }
  ...
  notInCollection = (collectionId: string, query: string = "") => {
    const memberships = filter(this.rootStore.memberships.orderedData, member => member.collectionId === collectionId);
    const userIds = memberships.map(member => member.userId);
    const users = filter(this.activeOrInvited, user => !userIds.includes(user.id));
  ...
  notInGroup = (groupId: string, query: string = "") => {
    const memberships = filter(this.rootStore.groupMemberships.orderedData, member => member.groupId === groupId);
    const userIds = memberships.map(member => member.userId);
    const users = filter(this.activeOrInvited, user => !userIds.includes(user.id));
    if (!query) return users;
    return queriedUsers(users, query);
  ...
  inGroup = (groupId: string, query: string) => {
    const groupMemberships = filter(this.rootStore.groupMemberships.orderedData, member => member.groupId === groupId);
...
function queriedUsers(users, query) {
  return filter(users, user => user.name.toLowerCase().includes(query.toLowerCase()));
}