...
export default class RevisionsStore extends BaseStore<Revision> {
  ...
  getDocumentRevisions(documentId: string): Revision[] {
    let revisions = filter(this.orderedData, {
      documentId
    });
    ...
    // isn't one
    if (latestRevision && document && latestRevision.createdAt !== document.updatedAt) {
      revisions.unshift(new Revision({
        id: "latest",
  ...
  @action
  fetch = async (id: string, options?: FetchOptions): Promise<Revision | null | undefined> => {
  ...
  @action
  fetchPage = async (options: PaginationParams | null | undefined): Promise<any> => {
    this.isFetching = true;
