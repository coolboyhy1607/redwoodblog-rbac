...
export default class CollectionsStore extends BaseStore<Collection> {
  ...
  get active(): Collection | null | undefined {
    return this.rootStore.ui.activeCollectionId ? this.data.get(this.rootStore.ui.activeCollectionId) : undefined;
  }

  @computed
  get orderedData(): Collection[] {
    let collections = Array.from(this.data.values());
    collections = collections.filter(collection => collection.deletedAt ? false : true);
  ...
  get pathsToDocuments(): DocumentPath[] {
    ...
    const travelDocuments = (documentList, collectionId, path) => documentList.forEach(document => {
      const {
        id,
        title,
        url
